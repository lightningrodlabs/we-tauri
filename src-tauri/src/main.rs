...
use state::{LaunchedState, WeError, WeResult};
                ...
                .on_web_resource_request(move |request, mut response| {
                    if request.uri().starts_with("https://uhcek") {
                        tauri::async_runtime::block_on(async {
                            ...
                            if let Err(err) =
                                handle_request(&m.web_app_manager, request, &mut response).await
                            {
...
async fn handle_request(
    ...
    let applet_id =
        get_applet_id_from_lowercase(lowercase_applet_id, web_app_manager.admin_interface_port())
            .await?;
