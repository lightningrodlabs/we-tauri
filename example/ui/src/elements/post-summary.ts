...
export class PostSummary extends LitElement {
   ...
   * @internal
   */
  _post = new StoreSubscriber(this, () => this.postsStore.posts.get(this.postHash));
  ...
  renderPost() {
    switch (this._post.value.status) {
      ...
      case "complete":
        if (!this._post.value.value) return html`<span>${msg("The requested post doesn't exist")}</span>`;
