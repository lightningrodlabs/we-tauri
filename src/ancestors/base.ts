...
import { Dark as theme } from '@neighbourhoods/design-system-styles';
...
export class NHComponent extends ScopedRegistryHost(LitElement) {
  static styles: CSSResult|CSSResult[] = [css`${unsafeCSS(theme)}`];
}
export class NHComponentShoelace extends ScopedRegistryHost(LitElement) {
  static styles: CSSResult|CSSResult[] = [css`${unsafeCSS(theme)}`];// , css`${unsafeCSS(shoelaceUIAdapter)}`];
}
export class NHComponentMaterial extends ScopedRegistryHost(LitElement) {
  static styles: CSSResult|CSSResult[] = [css`${unsafeCSS(theme)}`]//, css`${unsafeCSS(materialUIAdapter)}`];
}